...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!poId && user.role !== "admin")
      throw new SourcingAccessError(
        "Only global admins may create ad-hoc receipts",
      );
    ...
    const receive = async () =>
      prisma.$transaction(async (tx) => {
        ...
        if (!warehouse)
          throw new SourcingAccessError("Warehouse not found", 404);
        const po = poId
          ? await tx.purchaseOrder.findUnique({
              where: { id: poId },
              include: { items: true, sourcingOrder: true },
            })
          : null;
        if (poId && !po)
          throw new SourcingAccessError("Purchase order not found", 404);
        if (
          po &&
          (!["approved", "ordered", "shipping"].includes(po.status) ||
            (po.sourcingOrder && !["ordered", "shipping"].includes(po.status)))
        )
          throw new SourcingAccessError(
            `PO must be ordered or shipping to receive (current: ${po.status})`,
            409,
          );
        if (po?.workspaceId) {
          await requireWorkspaceRole(user, po.workspaceId, [
            "admin",
            "warehouse",
          ]);
          ...
          if (
            warehouse.workspaceId !== po.workspaceId &&
            !compatibleLegacyWarehouse
          )
            throw new SourcingAccessError(
              "Warehouse must belong to the purchase order workspace",
              400,
            );
        } else {
          if (warehouse.workspaceId || (po && po.userId !== warehouse.userId)) {
            throw new SourcingAccessError(
              "Legacy purchase order and warehouse must have the same owner",
              400,
            );
          }
          if (!dataScope.ownerIds.includes(warehouse.userId))
            throw new SourcingAccessError(
              "Warehouse is not available to this user",
            );
        }
        if (po && items.some((item) => !item.poItemId))
          throw new SourcingAccessError(
            "Each PO receipt line requires a purchase-order item",
            400,
          );
        if (po) {
          ...
          await tx.purchaseOrder.update({
            where: { id: po.id },
            data: { updatedAt: new Date(), updatedBy: user.id },
          });
        }
        ...
        for (const item of items) {
          ...
          if (
            !product ||
            (po?.workspaceId && product.workspaceId !== po.workspaceId)
          )
            throw new SourcingAccessError(
          ...
          if (
            !po?.workspaceId &&
            (product.workspaceId ||
              product.userId !== warehouse.userId ||
              (po && product.userId !== po.userId))
          ) {
            throw new SourcingAccessError(
              `Product ${item.productId} is not in the legacy purchase order scope`,
              400,
            );
          }
          ...
          if (po) {
            ...
            if (!poItem || poItem.productId !== item.productId)
              throw new SourcingAccessError(
                "Receipt line does not match a PO item",
                400,
              );
            const newTotal = (pendingByPoItem.get(poItem.id) ?? 0) + total;
            if (poItem.quantityReceived + newTotal > poItem.quantity)
              throw new SourcingAccessError(
                `Receipt exceeds ordered quantity for ${poItem.sku ?? poItem.productName}`,
                409,
              );
            pendingByPoItem.set(poItem.id, newTotal);
            poItemStatus = {
              quantityOrdered: poItem.quantity,
              quantityReceived: poItem.quantityReceived + newTotal,
              fullyReceived:
                poItem.quantityReceived + newTotal === poItem.quantity,
            };
            receiptItems.push({
              ...
              inspectionPhotoUrls: item.inspectionPhotoUrls,
              notes: item.notes,
            });
          }
          ...
          if (accepted > 0) {
            ...
            await tx.stockAllocation.upsert({
              ...
              update: {
                quantity: { increment: acceptedBigInt },
                updatedAt: new Date(),
              },
              create: {
                productId: product.id,
                warehouseId,
                quantity: acceptedBigInt,
                userId: user.id,
            ...
            const updated = await tx.product.update({
              where: { id: product.id },
              data: { quantity: { increment: acceptedBigInt } },
            ...
            await tx.stockMovement.create({
              data: {
                ...
                quantity: acceptedBigInt,
                type: "in",
                sourceType: po ? "purchase_order" : "ad_hoc",
                sourceId: po?.id,
          ...
          results.push({
            ...
            sku: product.sku,
            quantity: accepted,
            newStockLevel: Number(stock),
            poItemStatus,
          });
        }
        if (po) {
          for (const [poItemId, quantity] of pendingByPoItem)
          ...
          if (po.workspaceId) {
            ...
            const allocations = allocateLandedCost(
              actualLandedCostMyr,
              receiptItems.map((item) => item.acceptedQuantity),
            );
            await tx.purchaseReceipt.create({
              data: {
                workspaceId: po.workspaceId,
                purchaseOrderId: po.id,
                sourcingOrderId: po.sourcingOrder?.id,
                sourcingCaseId: po.sourcingOrder?.caseId,
                warehouseId,
                receivedById: user.id,
                notes,
                actualFreightMyr,
                ...
                actualLandedCostMyr,
                items: {
                  create: receiptItems.map((item, index) => ({
                    ...item,
                    allocatedLandedCostMyr: allocations[index] ?? 0,
                    unitLandedCostMyr: item.acceptedQuantity
                      ? (allocations[index] ?? 0) / item.acceptedQuantity
                      : null,
                  })),
                },
              },
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        ...
        if (
          ...
          !/write conflict|transaction.*conflict|P2034/i.test(error.message)
        )
          throw error;
      }
    }
    if (!result) throw new Error("Receiving transaction did not complete");
    void invalidateAllServerCaches().catch((error) =>
      logger.error("[Receiving] Cache invalidation failed", error),
    );
