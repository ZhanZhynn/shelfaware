...
import {
  deleteStoredSourcingAttachment,
  storeSourcingAttachment,
} from "@/lib/sourcing/attachment-storage";
...
async function caseForUser(request: NextRequest, id: string) {
  ...
  const sourcingCase = await prisma.sourcingCase.findUnique({
    where: { id },
    select: { id: true, workspaceId: true, assignedToId: true },
  });
  if (!sourcingCase)
    throw new SourcingAccessError("Sourcing case not found", 404);
  await requireWorkspaceRole(user, sourcingCase.workspaceId, [
    "admin",
    "sourcer",
  ]);
  ...
  return NextResponse.json(
    {
      error:
        error instanceof Error
          ? error.message
          : "Sourcing attachment request failed",
    },
    { status },
  );
}
...
export async function GET(
  ...
  try {
    const { user, sourcingCase } = await caseForUser(
      request,
      (await params).id,
    );
    const attachments = await prisma.sourcingAttachment.findMany({
      where: { caseId: sourcingCase.id },
...
export async function POST(
  ...
  try {
    ...
    const limited = await withRateLimit(
      ...
      `sourcing:attachments:${user.id}`,
    );
    if (limited) return limited;
    const formData = await request.formData();
    const file = formData.get("file");
    const quoteId = formData.get("quoteId");
    const caseVariantId = formData.get("caseVariantId");
    if (!(file instanceof File))
      return NextResponse.json(
        { error: "A file is required" },
        { status: 400 },
      );
    if (quoteId !== null && typeof quoteId !== "string")
    ...
    if (caseVariantId) {
      const variant = await prisma.sourcingCaseVariant.findFirst({
        where: { id: caseVariantId, caseId: sourcingCase.id },
        select: { id: true },
      });
      if (!variant)
        return NextResponse.json(
          { error: "Variant not found for this sourcing case" },
          { status: 400 },
        );
    }
    ...
    const fileId = await storeSourcingAttachment(
      file.name,
      file.type,
      Buffer.from(await file.arrayBuffer()),
    );
    try {
      const attachment = await prisma.sourcingAttachment.create({
        data: {
      ...
      const saved = await prisma.sourcingAttachment.update({
