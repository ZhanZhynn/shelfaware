...
function failure(error: unknown) {
  ...
  return NextResponse.json(
    {
      error: error instanceof Error ? error.message : "Sourcing request failed",
    },
    { status },
  );
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const access = await requireWorkspaceRole(user, workspaceId, [
      "admin",
      "sourcer",
    ]);
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(normalizeSourcingListCase(sourcingCase), {
      status: 201,
    });
  } catch (error) {
    return failure(error);
