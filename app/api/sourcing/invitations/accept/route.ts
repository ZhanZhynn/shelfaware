...
export async function POST(request: NextRequest) { try { const limited = await withRateLimit(request, defaultRateLimits.auth); if (limited) return limited; const body = schema.parse(await request.json()); await acceptWorkspaceInvitation(body.token, body.name, body.password); return NextResponse.json({ status: "accepted" }, { status: 201 }); } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to accept invitation" }, { status: error instanceof z.ZodError ? 400 : 409 }); } }
