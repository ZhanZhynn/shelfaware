...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!source.workspaceId && (!dataScope.ownerIds.includes(source.userId) || !dataScope.ownerIds.includes(target.userId))) {
        throw new SourcingAccessError("Source or target supplier was not found", 404);
      }
