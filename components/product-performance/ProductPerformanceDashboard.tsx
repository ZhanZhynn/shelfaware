...
const coverageBadgeClass = (pct: number | null | undefined) => {
...
export function dateFor(days: number, now = new Date()) { const to = new Date(now); const from = new Date(to); from.setDate(to.getDate() - (days - 1)); return { dateFrom: from.toISOString().slice(0, 10), dateTo: to.toISOString().slice(0, 10) }; }
export function isValidDate(value: string | null): value is string { if (!value || !/^\d{4}-\d{2}-\d{2}$/.test(value)) return false; return new Date(`${value}T00:00:00.000Z`).toISOString().slice(0, 10) === value; }
...
export default function ProductPerformanceDashboard() {
  ...
  const fallbackRange = dateFor(period); const requestedDateFrom = params.get("dateFrom"); const requestedDateTo = params.get("dateTo"); const dateFrom = isValidDate(requestedDateFrom) ? requestedDateFrom : fallbackRange.dateFrom; const dateTo = isValidDate(requestedDateTo) ? requestedDateTo : fallbackRange.dateTo;
  useEffect(() => {
    if ((!requestedDateFrom || isValidDate(requestedDateFrom)) && (!requestedDateTo || isValidDate(requestedDateTo))) return;
    const next = new URLSearchParams(params.toString());
    if (requestedDateFrom && !isValidDate(requestedDateFrom)) next.set("dateFrom", fallbackRange.dateFrom);
    if (requestedDateTo && !isValidDate(requestedDateTo)) next.set("dateTo", fallbackRange.dateTo);
    router.replace(`${pathname}?${next}`);
  }, [fallbackRange.dateFrom, fallbackRange.dateTo, params, pathname, requestedDateFrom, requestedDateTo, router]);
  const { data, isLoading, error } = useProductPerformance({ dateFrom, dateTo });
  ...
  const columns: ColumnDef<ProductPerformanceRow>[] = [
    ...
    { accessorKey: "marketplaceNormalizedUnits", header: "Marketplace", cell: ({ row }) => {
      const units = row.original.marketplaceNormalizedUnits;
      const coverage = row.original.marketplaceCoverage;
      if (units == null) return <span className="text-muted-foreground">No marketplace data</span>;
      return <><span>{units} normalized units</span>{coverage && <span className={`mt-1 block w-fit rounded px-1.5 py-0.5 text-xs font-medium ${coverageBadgeClass(coverage.mappingPercent)}`}>{coverage.mappingPercent}% mapped ({coverage.mappedOffers}/{coverage.totalOffers})</span>}</>;
    } },
  ];
  ...
  return <div className="space-y-5"><div><h1 className="text-2xl font-bold">Product Performance</h1><p className="text-muted-foreground">Recommendations use WMS inventory, sales, supplier lead time, and review-quality signals. Marketplace demand from shared offer/SalesSku/recipe facts is shown when available. ABC tier and mapped Shopee visibility are informational filters only.</p><Link className="mt-2 inline-block text-sm font-medium underline" href={`/admin/inventory/sku-mapping?dateFrom=${dateFrom}&dateTo=${dateTo}`}>View shared cross-channel SKU attribution</Link></div>
    ...
    <div className="flex flex-wrap gap-2">{presets.map((item) => <Button key={item.days} size="sm" variant={period === item.days ? "default" : "outline"} onClick={() => setPreset(item.days)}>{item.label}</Button>)}<label className="sr-only" htmlFor="product-performance-date-from">From date</label><Input id="product-performance-date-from" className="w-40" type="date" value={dateFrom} onChange={(event) => setParam("dateFrom", event.target.value)} /><label className="sr-only" htmlFor="product-performance-date-to">To date</label><Input id="product-performance-date-to" className="w-40" type="date" value={dateTo} onChange={(event) => setParam("dateTo", event.target.value)} /><select className="rounded border bg-background px-2 text-sm" value={tier} onChange={(event) => setParam("tier", event.target.value)} title="ABC tier is descriptive and does not affect recommendations."><option value="">All tiers</option><option>A</option><option>B</option><option>C</option></select><label className="sr-only" htmlFor="product-performance-visibility">Shopee visibility</label><select id="product-performance-visibility" className="rounded border bg-background px-2 text-sm" value={channel} onChange={(event) => setParam("channel", event.target.value)}><option value="all">All visibility</option><option value="mapped">Mapped Shopee</option><option value="needs-mapping">Needs mapping</option></select><label className="sr-only" htmlFor="product-performance-search">Search products</label><Input id="product-performance-search" className="w-56" placeholder="Search name or SKU" value={search} onChange={(event) => setParam("search", event.target.value)} /></div>
    <div className="overflow-x-auto rounded-lg border"><Table><TableHeader>{table.getHeaderGroups().map((group) => <TableRow key={group.id}>{group.headers.map((header) => <TableHead key={header.id}><button className="flex items-center gap-1" onClick={header.column.getToggleSortingHandler()}>{flexRender(header.column.columnDef.header, header.getContext())}{header.column.getIsSorted() === "asc" ? <ChevronUp size={14} /> : header.column.getIsSorted() === "desc" ? <ChevronDown size={14} /> : null}</button></TableHead>)}<TableHead>Details</TableHead></TableRow>)}</TableHeader><TableBody>{table.getRowModel().rows.map((row) => <Fragment key={row.id}><TableRow>{row.getVisibleCells().map((cell) => <TableCell key={cell.id}>{flexRender(cell.column.columnDef.cell, cell.getContext())}</TableCell>)}<TableCell><Button variant="ghost" size="sm" onClick={() => setExpanded(expanded === row.id ? null : row.id)}>{expanded === row.id ? "Hide" : "Show"}</Button></TableCell></TableRow>{expanded === row.id && <TableRow><TableCell colSpan={columns.length + 1} className="bg-muted/40 text-sm">Velocity: {row.original.dailyVelocity ?? "-"} units/day. Lead time: {row.original.supplierLeadTimeDays ?? "not recorded"}. Safety stock: {data?.defaults.safetyDays} days. Inbound PO quantity: not included until partially received PO quantities can be safely reconciled. Suggested quantity: {row.original.suggestedQuantity ?? "-"}. Tier: {row.original.tier ?? "unranked"} (informational). Shopee: {row.original.shopeeCoverage} (informational). {row.original.reviewQuality ? ` Approved review quality: ${row.original.reviewQuality.averageRating.toFixed(1)}/5 from ${row.original.reviewQuality.count}.` : " No approved review-quality signal."}{row.original.marketplaceNormalizedUnits != null ? ` Marketplace: ${row.original.marketplaceNormalizedUnits} normalized units from ${row.original.contributingSalesSkus?.length ?? 0} contributing Sales SKU(s).` : ""}</TableCell></TableRow>}</Fragment>)}{!rows.length && <TableRow><TableCell colSpan={columns.length + 1} className="py-12 text-center text-muted-foreground">No products match this queue or filter.</TableCell></TableRow>}</TableBody></Table></div></div>;
}
