...
export const ANALYTICS_CALCULATION_VERSION = "v3-provenance";
...
export async function getMarketplaceAnalytics(platform: MarketplacePlatform, session: Parameters<typeof marketplaceOwnerIds>[0], params: URLSearchParams, metric = "summary"): Promise<MarketplaceAnalyticsResult> {
  ...
  if (platform === "shopee") {
    ...
    shopCount = shops.length;
    const rows = await prisma.lazadaOrder.findMany({ where: { shopId: { in: shops.map((shop) => shop.id) }, ...fallbackDateFilter("lazadaCreatedAt") }, include: { items: true } });
    orders = rows.map((row) => ({ id: row.id, shopId: row.shopId, platform, currency: row.currency ?? "unknown", createdAt: orderDate(row), status: row.orderStatus, buyerId: null, buyerDisplayName: null, financialQuality: row.financialQuality as NormalizedOrderFinancials["financialQuality"], grossSales: row.totalAmount, sellerDiscount: null, platformDiscount: null, refund: null, buyerShippingCredit: row.shippingFee, platformSubsidy: null, marketplaceFees: null, paymentFees: null, sellerShipping: null, returnShipping: null, otherCharges: null, settledProceeds: null, settledProceedsVerified: false, items: row.items.map((item) => ({ id: item.id, productId: item.skuId ?? item.sellerSku, productName: item.productName, sku: item.sellerSku, quantity: item.quantity, grossSales: null, refund: null, financialQuality: row.financialQuality as NormalizedOrderFinancials["financialQuality"] })) }));
    ...
    orders = rows.map((row) => { const buyerId = row.buyerUserId ?? row.buyerEmail; return { id: row.id, shopId: row.shopId, platform, currency: (row.payment as { currency?: string } | null)?.currency ?? "unknown", createdAt: orderDate(row), status: row.orderStatus, buyerId, buyerDisplayName: buyerPseudonym(row.shopId, buyerId), financialQuality: row.financialQuality as NormalizedOrderFinancials["financialQuality"], grossSales: null, sellerDiscount: null, platformDiscount: null, refund: null, buyerShippingCredit: null, platformSubsidy: null, marketplaceFees: null, paymentFees: null, sellerShipping: null, returnShipping: null, otherCharges: null, settledProceeds: null, settledProceedsVerified: false, items: row.items.map((item) => ({ id: item.tiktokOrderLineItemId, productId: item.skuId ?? item.productId, productName: item.productName, sku: item.sellerSku, quantity: null, grossSales: null, refund: null, financialQuality: row.financialQuality as NormalizedOrderFinancials["financialQuality"] })) }; });
  } else {
    ...
    shopCount = shops.length;
    const rows = await prisma.shopifyOrder.findMany({ where: { shopId: { in: shops.map((shop) => shop.id) }, test: false, ...fallbackDateFilter("shopifyCreatedAt") }, include: { items: true } });
    orders = rows.map((row) => { const buyerId = row.customerEmail; return { id: row.id, shopId: row.shopId, platform, currency: row.currency, createdAt: orderDate(row), status: row.orderStatus, buyerId, buyerDisplayName: buyerPseudonym(row.shopId, buyerId), financialQuality: row.financialQuality as NormalizedOrderFinancials["financialQuality"], grossSales: null, sellerDiscount: null, platformDiscount: null, refund: null, buyerShippingCredit: null, platformSubsidy: null, marketplaceFees: null, paymentFees: null, sellerShipping: null, returnShipping: null, otherCharges: null, settledProceeds: null, settledProceedsVerified: false, items: row.items.map((item) => ({ id: item.shopifyLineId, productId: item.variantId, productName: item.name, sku: item.sku, quantity: item.currentQuantity, grossSales: null, refund: null, financialQuality: row.financialQuality as NormalizedOrderFinancials["financialQuality"] })) }; });
  }
...
function buildResponse(orders: NormalizedOrderFinancials[], currency: string, shopCount: number, granularity: "daily" | "weekly" | "monthly", metric: string, financeReady: boolean) {
  ...
  const products = new Map<string, { productId: string | null; productName: string; sku: string | null; quantity: number | null; grossSales: null; estimatedProfit: null }>();
  if (metric === "products") for (const order of orders) for (const item of order.items) { const key = `${order.shopId}:${item.productId ?? `unmapped:${item.sku ?? item.id}`}`; const prior = products.get(key); products.set(key, { productId: item.productId, productName: item.productName, sku: item.sku, quantity: prior?.quantity === null || item.quantity === null ? null : (prior?.quantity ?? 0) + item.quantity, grossSales: null, estimatedProfit: null }); }
  ...
  const trend = metric === "revenue-trend" ? [...new Set(orders.map((order) => bucket(order.createdAt)))].map((date) => ({ date, grossSales: null, netSales: null, estimatedProfit: null, orders: orders.filter((order) => bucket(order.createdAt) === date).length })) : [];
  return { summary: { totalOrders: orders.length, certifiedFinancialOrders: metric === "profit" ? profit.orderCount : 0, netSales: null, averageOrderValue: null, shopCount }, profit, buyers, clv, products: [...products.values()].sort((a, b) => `${a.productName}\u0000${a.sku ?? ""}`.localeCompare(`${b.productName}\u0000${b.sku ?? ""}`)), revenueTrend: trend };
}
